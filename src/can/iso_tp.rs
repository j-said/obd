...
/// FC response block size: 0 = unlimited — safe because handle_ff rejects ff_dl > 256.
const FC_BS: u8 = 0;
...
impl<D: AsyncCanDriver> IsoTpHandler<D> {
    ...
    async fn transmit_multi(&self, tx_id: Id, fc_id: Id, data: &[u8]) -> Result<(), IsoTpError> {
        ...
        // await FC(CTS) — receive_fc handles WAIT/OVFLW/reserved internally
    ...
    async fn receive_single(&self, target_id: Id) -> Result<Vec<u8, 256>, IsoTpError> {
        ...
        let first = self.receive_first_frame(&mut state, target_id).await?;
        if first {
    ...
    async fn receive_cfs(
        ...
        loop {
            ...
            let frame = with_timeout(deadline - now, self.driver.receive())
                ...
                .map_err(|_| IsoTpError::DriverError)?;
            if frame.id() == target_id {
                if self.process_frame(state, &frame).await? {
                    return Ok(core::mem::replace(&mut state.buffer, Vec::new()));
                }
    ...
    async fn handle_ff(
        ...
        let min_ff_dl = 8 - self.pci_offset();
        if ff_dl < min_ff_dl {
            return Ok(false);
        }
        let fc_id = self.get_fc_id(id)?;
        if ff_dl > 256 {
            self.send_flow_control(fc_id, FlowStatus::Overflow, 0, 0)
                .await?;
            return Err(IsoTpError::BufferOverflow);
        }
        state.rx_dl = ff_dl;
        ...
        self.send_flow_control(fc_id, FlowStatus::ContinueToSend, FC_BS, FC_STMIN)
            .await?;
        Ok(false)
    }
    ...
    fn handle_cf(&self, state: &mut TransferState, d: &[u8]) -> Result<bool, IsoTpError> {
        // need at least PCI byte + 1 data byte before indexing d[1]
        if d.len() < 2 {
            return Ok(false);
        }
        ...
        let to_copy = core::cmp::min(state.rx_dl - state.buffer.len(), d.len() - 1);
        state
            .buffer
...
// TODO: configurable ID mapping — get_fc_id / is_valid_resp hardcoded to OBD-II offsets; needs to be injectable for non-OBD2 use
