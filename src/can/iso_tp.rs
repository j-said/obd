...
use super::{CanManager, is_extended};
...
impl<'a> IsoTpHandler<'a> {
    ...
    async fn transmit_sf(&self, id: u32, data: &[u8], ext: bool) -> Result<(), IsoTpError> {
        let mut tx = [0xAA; 8]; // Padding byte 0xAA (стандартний заповнювач)

        if data.len() > 7 {
            return Err(IsoTpError::BufferOverflow);
        }

        // PCI byte: Single Frame (0x0) | Length
        tx[0] = data.len() as u8;
        ...
        let can_id = if ext {
            ...
            // Перевірка, чи ID влазить у 11 біт
            if id > 0x7FF {
        ...
        let frame = EspTwaiFrame::new(can_id, &tx).ok_or(IsoTpError::CanError)?;
    ...
    async fn receive_single(&self, target_id: u32) -> Result<Vec<u8, 64>, IsoTpError> {
        ...
        with_timeout(Duration::from_millis(1000), async {
            loop {
                ...
                // Отримуємо поточний режим для перевірки
                let is_ext_mode = is_extended();
                let frame_is_ext = matches!(frame.id(), Id::Extended(_));

                // Ігноруємо кадри, що не відповідають поточному режиму адресації
                if is_ext_mode != frame_is_ext {
                    continue;
                }

                // Витягуємо raw ID для порівняння
                ...
                // Розбір PCI байту (Protocol Control Information)
                match d[0] >> 4 {
                    0 => {
                        // Single Frame
                    ...
                    1 => {
                        ...
                        let fc_id = if is_ext_mode {
                            // Extended: 18DA F1 10 (req) -> 18DA 10 F1 (resp) -> FC to 18DA F1 10
                            target_id
                        } else {
                            target_id - 8
                        };
        ...
        let _ = with_timeout(total_guard, async {
            loop {
                match with_timeout(inter_frame, self.manager.receive()).await {
                        ...
                        if valid_resp {
                            ...
                            if !d.is_empty() {
                                ...
                                if len <= 7 {
                                    ...
                                    // Уникаємо дублікатів від одного й того ж ECU
                    ...
                    _ => break, // Timeout між кадрами - вважаємо, що всі відповіли
                }
            }
        })
        .await;
    ...
    async fn send_flow_control(&self, request_id: u32) -> Result<(), IsoTpError> {
        ...
        let frame = EspTwaiFrame::new(can_id, &fc).ok_or(IsoTpError::CanError)?;
