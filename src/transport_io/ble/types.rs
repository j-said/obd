...
pub(crate) type BleResources = HostResources<
    MyPacketPool,
...
pub(crate) type MyBleStack<'a, C> = Stack<'a, C, MyPacketPool>;             // Білдер
pub(crate) type MyBleHost<'a, C> = Host<'a, C, MyPacketPool>;               // Результат build()
pub(crate) type MyBlePeripheral<'a, C> = Peripheral<'a, C, MyPacketPool>;   // GAP інтерфейс
pub(crate) type MyBleRunner<'a, C> = Runner<'a, C, MyPacketPool>;           // Процес стека
